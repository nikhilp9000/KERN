...
def get_obj_cooccurrence_mat(train_data=VG(mode='train', filter_duplicate_rels=False, num_val_im=350)):
    """
    ...
    for ex_ind in range(len(train_data)):
        ...
        for (i, j) in inds:
            mat[gt_classes_list[i], gt_classes_list[j]] += 1
...
def get_counts(train_data=VG(mode='train', filter_duplicate_rels=False, num_val_im=350), must_overlap=True):
    """
