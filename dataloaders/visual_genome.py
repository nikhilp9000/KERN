...
class VG(Dataset):
    def __init__(self, mode, roidb_file=VG_SGG_FN, dict_file=VG_SGG_DICT_FN,
                 image_file=IM_DATA_FN, filter_empty_rels=True, num_im=7500, num_val_im=350,
        ...
        print(len(self.split_mask))
        print(len(np.where(self.split_mask)[0]))
...
def load_graphs(graphs_file, mode='train', num_im=7500, num_val_im=0, filter_empty_rels=True,
    ...
    print(len(split_mask))
    print(split_mask)
    split_mask=split_mask[:7500]
    print(len(split_mask))
    print(split_mask)
